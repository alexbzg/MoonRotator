...
*/
    ...
    * The range of event constant values reserved for UI Automation property-changed event identifiers. * For more information, see Allocation of WinEvent IDs. */
    ...
    public class WindowResizeMoveEventArgs: EventArgs {
        public RECT newWindowRect;
    }

    public class FFPlayWindow : IDisposable
    {
        ...
        private Dictionary<WinEvents, IntPtr> winEventHooks = new Dictionary<WinEvents, IntPtr>();

        internal EventHandler<WindowResizeMoveEventArgs> windowResizeMove;
        internal EventHandler windowClose;

        private WinEventDelegate _winEventCallbackResizeMove;
        private WinEventDelegate _winEventCallbackDestroy;

        public FFPlayWindow(string source, IntPtr parent, string ffplayPath = "ffplay.exe") {
            ffplay = new Process();
            ffplay.StartInfo.FileName = ffplayPath;
            ffplay.StartInfo.Arguments = $"{source}";
            ...
            _winEventCallbackResizeMove = new WinEventDelegate(winEventCallbackResizeMove);
            winEventHooks[WinEvents.EVENT_SYSTEM_MOVESIZEEND] = SetWinEventHook(WinEvents.EVENT_SYSTEM_MOVESIZEEND, WinEvents.EVENT_SYSTEM_MOVESIZEEND, IntPtr.Zero, _winEventCallbackResizeMove,
        ...
        private void winEventCallbackResizeMove(IntPtr hWinEventHook, uint eventType,
            IntPtr hwnd, int idObject, int idChild, uint dwEventThread, uint dwmsEventTime)
        {
            ffplay.Refresh();
            RECT rect = new RECT();
            if (GetWindowRect(ffplay.MainWindowHandle, ref rect))
            {
                windowResizeMove?.Invoke(this, new WindowResizeMoveEventArgs { newWindowRect = rect });
            }
        }

        private void winEventCallbackDestroy(IntPtr hWinEventHook, uint eventType,
            IntPtr hwnd, int idObject, int idChild, uint dwEventThread, uint dwmsEventTime)
        {
            unhookEvents();
            ffplay = null;
        ...
        public void moveWindow(RECT windowRect)
        {
            MoveWindow(ffplay.MainWindowHandle, windowRect.left, windowRect.top, windowRect.right - windowRect.left, windowRect.bottom - windowRect.top, true);
        }

        private void unhookEvents()
        {
            foreach (IntPtr handle in winEventHooks.Values)
